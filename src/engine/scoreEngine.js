...
import { scoreStriper } from './striperScore.js';
import { scoreSpot } from './spotScore.js';
...
export function classifyInflow(cfs) {
  ...
  if (cfs < 500) return 'Very Low';
  if (cfs < 2000) return 'Normal';
  if (cfs < 5000) return 'High';
  return 'Flood/Turbid';
}

export function estimateWaterTemp(hourlyAirF, currentIndex) {
  // Rolling 72-hour average air temp minus a lag offset.
  if (!hourlyAirF || hourlyAirF.length === 0) return 64; // neutral fallback
  const window = 72;
  const start = Math.max(0, currentIndex - window);
  const slice = hourlyAirF.slice(start, currentIndex + 1).filter(v => v != null);
  if (!slice.length) return 64;
  const avg = slice.reduce((a, b) => a + b, 0) / slice.length;
  const wt = avg - 4;
  return Math.max(38, Math.min(88, Math.round(wt)));
}
...
export function scoreAll(conditions) {
  return {
    striper: scoreStriper(conditions),
    spot: scoreSpot(conditions),
...
export function projectHourly({
  ...
  if (!weather || !weather.hourly) {
    return { striper: [], spot: [], crappie: [] };
  }

  const out = { striper: [], spot: [], crappie: [] };
  ...
  if (base < 0) return { striper: [], spot: [], crappie: [] };

  for (let h = 0; h < 24; h++) {
    const i = base + h;
    const t = times[i];
    if (!t) {
      out.striper.push(null); out.spot.push(null); out.crappie.push(null);
      continue;
    }
    ...
    const s = scoreAll(c);
    out.striper.push(s.striper.score);
    out.spot.push(s.spot.score);
