...
const SPECIES_NAMES = { striper: 'Striped Bass', spot: 'Spotted Bass', crappie: 'Crappie' };
const ZONE_NAMES = { upper: 'Upper Lake', mid: 'Mid Lake', deep: 'Southern Deep' };
...
export default function App() {
  ...
  const liveRows = useMemo(() => {
    if (!derived) return [];
    return [
      { name: 'Air Temp',      value: fmt(derived.airTemp, '°F'),         status: derived.airTemp >= 50 && derived.airTemp <= 80 ? 'Favorable' : 'Neutral' },
      { name: 'Water Temp',    value: `~${fmt(derived.waterTemp, '°F')}`, status: derived.waterTemp >= 58 && derived.waterTemp <= 72 ? 'Favorable' : derived.waterTemp > 80 ? 'Unfavorable' : 'Neutral' },
      ...
      { name: 'Cloud Cover',   value: fmt(derived.clouds, '%'),           status: derived.clouds > 75 ? 'Favorable' : derived.clouds >= 30 ? 'Neutral' : 'Neutral' },
      ...
      { name: 'Inflow',        value: gauge.data?.discharge != null ? `${Math.round(gauge.data.discharge)} cfs (${derived.inflowClass})` : '--', status: derived.inflowClass === 'Normal' ? 'Favorable' : derived.inflowClass === 'Flood/Turbid' ? 'Unfavorable' : 'Neutral' },
      ...
      { name: 'Solunar',       value: solunarStatus(astro.solunarPeriods, new Date()), status: solunarStatus(astro.solunarPeriods, new Date()) === 'Major' ? 'Favorable' : solunarStatus(astro.solunarPeriods, new Date()) === 'Minor' ? 'Neutral' : 'Neutral' },
    ];
    ...
    <div className="min-h-screen text-body">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-baseline justify-between flex-wrap gap-2 mb-3">
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-heading uppercase tracking-wider">
              Lanier <span className="text-accent">Bite Score</span>
            </h1>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-5 flex flex-col gap-5">
        ...
        <footer className="text-center text-xs text-body/50 py-4">
          Data: Open-Meteo · USGS gauge 02334480 · Lake Lanier 34.2015°N, 83.9558°W
        </footer>
      </main>
